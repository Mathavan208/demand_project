...
const WorkshopDetail = () => {
  ...
  useEffect(() => {
    const foundWorkshop = workshopData.find(w => w.id === parseInt(id));
    if (foundWorkshop) {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-deep-blue">Workshop Not Found</h1>
          <Link to="/workshops" className="inline-block mt-4 text-purple-blue hover:underline">
            Back to Workshops
          </Link>
        </div>
      </div>
            ...
            {workshop.status === 'available' ? (
              ...
              <div className="inline-block p-4 border border-yellow-200 rounded-lg bg-yellow-50">
                <p className="font-medium text-yellow-800">
                  Registration opens on {workshop.registrationOpens}!
                </p>
              </div>
            )}
          </div>
